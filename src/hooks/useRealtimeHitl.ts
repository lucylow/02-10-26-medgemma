/**
 * Real-time HITL WebSocket hook for PediScreen AI
 * Live clinician collaboration, case queue updates, and streaming notifications.
 * Set VITE_WS_URL in .env (e.g. ws://localhost:5001 or wss://api.pediscreen.ai)
 */
...
function getWsBaseUrl(): string {
  ...
  return import.meta.env.DEV ? "ws://localhost:5001" : "";
}
...
export function useRealtimeHitl(
  ...
  useEffect(() => {
    const handleOnline = () => {
      if (navigator.onLine && clinicId) connectWebSocket();
    };
    window.addEventListener("online", handleOnline);
    return () => window.removeEventListener("online", handleOnline);
  }, [clinicId, connectWebSocket]);

  useEffect(() => {
    const handleVisibility = () => {
      if (document.visibilityState === "visible" && clinicId) connectWebSocket();
    };
