...
export const RespiratoryRateWidget: React.FC = () => {
  const { current, ageMonths } = useVitalSigns();
  const range = getVitalRangeForAge(ageMonths, "respiratoryRate");

  const value = current?.respiratoryRate;
  const formatted =
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-slate-200 bg-white/90 px-4 py-3 shadow-sm">
      ...
      <div className={`mt-1 text-2xl font-bold ${status}`}>
        {formatted}
        <span className="ml-1 text-xs text-slate-500">br/min</span>
      </div>
      {range && (
        <span className="mt-1 text-[11px] text-slate-400">
          Range {range.min}–{range.max} br/min
        </span>
      )}
    </div>
  );
