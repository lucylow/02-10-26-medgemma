...
// Match screening API base: backend runs on 8000 by default
...
export type TechnicalWritingResponse = {
  mode: WritingMode;
  content: string;
  disclaimer: string;
};
...
export const generateTechnicalWriting = async (
  req: TechnicalWritingRequest
): Promise<TechnicalWritingResponse> => {
  const base = API_BASE.replace(/\/api\/?$/, '');
  const url = `${base}/api/writing/generate`;
...
export const generateStructuredWriting = async (
  ...
  const base = API_BASE.replace(/\/api\/?$/, '');
  const url = `${base}/api/writing/generate-structured`;
  const res = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
