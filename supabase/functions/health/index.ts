...
serve(async (req) => {
  ...
  try {
    const [dbCheck, aiGatewayCheck, metricsCheck, lastEventCheck, driftCheck, consentCheck, orgCheck] = await Promise.all([
      // DB connectivity + row count (screenings is required)
      (async () => {
      ...
      // Last-hour edge metrics summary (optional table)
      (async () => {
        try {
      ...
      // Last AI event timestamp (optional table)
      (async () => {
        try {
      ...
      // Latest PSI drift (optional table)
      (async () => {
        try {
      ...
      // Consent system check (optional table)
      (async () => {
        try {
      ...
      // Organizations check (optional table)
      (async () => {
        try {
