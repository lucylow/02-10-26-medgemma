...
/**
 * AI Usage Telemetry API — overview, model breakdown, errors, fallbacks.
 * Reads from ai_events table for rich analytics.
 */
...
function parseRange(range: string): number {
  ...
  if (!match) return 7 * 24 * 60; // default 7 days in minutes
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "overview") {
      return await handleOverview(since, windowMinutes);
    } else if (action === "models") {
      return await handleModels(since);
    } else if (action === "errors") {
      return await handleErrors(since);
    } else if (action === "fallbacks") {
      return await handleFallbacks(since);
    } else {
      return jsonResponse({ error: "Unknown action. Use: overview, models, errors, fallbacks" }, 400);
    }
  } catch (err) {
...
async function handleOverview(since: string, windowMinutes: number) {
  ...
  // Active connection check (any event in last 5 min)
  const fiveMinAgo = new Date(Date.now() - 5 * 60 * 1000).toISOString();
  const recentEvents = items.filter(e => e.timestamp >= fiveMinAgo);
  const activeConnection = recentEvents.length > 0;
...
async function handleModels(since: string) {
  ...
  (events || []).forEach(e => {
    ...
    if (!models[mid]) {
      models[mid] = {
        model_id: mid,
        provider: e.model_provider || "unknown",
        calls: 0, errors: 0, fallbacks: 0,
        totalLatency: 0, latencyCount: 0, totalCost: 0,
        adapters: new Set(),
  ...
  return jsonResponse({ models: result, since });
}
...
async function handleErrors(since: string) {
  ...
  return jsonResponse({ errors: events || [], since });
}
...
async function handleFallbacks(since: string) {
  ...
  (events || []).forEach(e => {
    const r = e.fallback_reason || "unknown";
    reasonCounts[r] = (reasonCounts[r] || 0) + 1;
  });

  return jsonResponse({ fallbacks: events || [], reason_summary: reasonCounts, since });
}
